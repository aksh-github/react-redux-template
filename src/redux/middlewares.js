...
//Ajax middleware (complex)

export const ServiceMiddleware2 = (store) => (next) => async (action) => {
    if (action.type !== 'API') {
        next(action);
        return;
    }
    ...
    const callValidator = (result, responseValidator) => {
        return responseValidator instanceof Function
            ? responseValidator(result)
            : true;
    };

    const callTransform = (result, transform) => {
        if (transform instanceof Function) return transform(result);
        else return result;
    };
    ...
    if (before instanceof Function) dispatch(before());
    ...
    try {
        ...
        if (validResponseFlag) {
            ...
            if (onFailure instanceof Function) dispatch(onFailure(result));
        }

        if (after instanceof Function) after(result);
        ...
        if (errorTransform instanceof Function) {
            ...
            if (onFailure instanceof Function) dispatch(onFailure(err));
        }

        if (after instanceof Function) after(err);

        return _result;
    }
};

//Ajax middleware (simple)
